...
INPUT_CSV = _BASE / "data/taptap_reviews_cleaned.csv"
OUTPUT_CSV = _BASE / "sentiment_results.csv"
ERROR_LOG  = _BASE / "sentiment_errors.log"

BATCH_SAVE_SIZE = 50      # 每50条落盘一次
SLEEP_BETWEEN_CALLS = 0.3 # 每次调用后sleep秒数(限流)
...
# ============ 主流程 ============
def main():
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 开始批量跑情感分析")
    ...
    try:
        ...
        for i, row in pbar:
            ...
            try:
                result = classify(content, client)
